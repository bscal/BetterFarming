...
import static me.bscal.betterfarming.BetterFarming.prefixPath;
...
public interface ResourceGenerateable
{
	...
	class Facing implements Block
	{
		...
		{
			this.top = BetterFarming.MOD_ID + ":block/" + top;
			this.front = BetterFarming.MOD_ID + ":block/" + front;
			this.side = BetterFarming.MOD_ID + ":block/" + side;
			this.bottom = BetterFarming.MOD_ID + ":block/" + bottom;
		}
		...
		{
			pack.addModel(model("minecraft:block/orientable").textures(
							textures().var("top", this.top).var("front", this.front).var("side", this.side).var("bottom", this.bottom)),
					prefixPath(id, "block"));
		}
	}
	...
	class FurnaceLike extends Facing
	{
		...
		public FurnaceLike(String top, String front, String side, String bottom, String top_on, String front_on, String side_on,
				String bottom_on)
		{
			super(top, front, side, bottom);
			this.top_on = BetterFarming.MOD_ID + ":block/" + top_on;
			this.front_on = BetterFarming.MOD_ID + ":block/" + front_on;
			this.side_on = BetterFarming.MOD_ID + ":block/" + side_on;
			this.bottom_on = BetterFarming.MOD_ID + ":block/" + bottom_on;
		}
		...
		public void generateBlockModel(RuntimeResourcePack pack, Identifier id)
		{
			...
			pack.addModel(model("minecraft:block/orientable").textures(
							textures().var("top", this.top_on).var("front", this.front_on).var("side", this.side_on).var("bottom",
									this.bottom_on)),
